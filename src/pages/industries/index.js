    ...
    return (
        <MainLayout title="Industries">
            <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-4">
                <h1 className="text-2xl font-semibold mb-8">Total Industries: {data.length}</h1>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {data.map((industry) => (
                        <div key={industry.id} className="bg-white p-6 rounded-lg shadow-lg">
                            <h2 className="text-lg font-bold mb-2">{industry.organizationName || 'Unnamed Industry'}</h2>
                            <p className="text-gray-600 text-sm">Email: {industry.email}</p>
                            <p className="text-gray-600 text-sm">ID: {industry.id}</p>
                            <Link href={`/industries/${industry.id}`} className="mt-4 inline-block bg-blue-500 text-white py-2 px-4 rounded hover:bg-blue-600 transition-colors">
